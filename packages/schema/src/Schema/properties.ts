...
export class Property<
  ...
  D extends M.Maybe<[DefaultConstraint, () => TypeOf<S>]>
> {
  readonly [PropertyTypeId]: PropertyTypeId = PropertyTypeId
  constructor(readonly _schema: S, readonly _optional: O, readonly _def: D) {}

  schema<S1 extends AnyS>(_: S1): Property<S1, O, M.Nothing> {
    return new Property(_, this._optional, M.nothing() as M.Nothing)
  }
  ...
  def(_: O extends Required ? () => TypeOf<S> : DefaultError): Property<S, O, M.Just<['both', () => TypeOf<S>]>>
  def<C extends DefaultConstraint>(
    _: O extends Required ? () => TypeOf<S> : DefaultError,
    constraint: C
  ): Property<S, O, M.Just<[C, () => TypeOf<S>]>>
  def(
    _: O extends Required ? () => TypeOf<S> : DefaultError,
    constraint?: DefaultConstraint
  ): Property<S, O, M.Just<[DefaultConstraint, () => TypeOf<S>]>> {
    return new Property(
      this._schema,
      this._optional,
      M.just([constraint ?? 'both', _]) as M.Just<[DefaultConstraint, () => TypeOf<S>]>
    )
  }

  undef(): Property<S, O, M.Nothing> {
    return new Property(this._schema, this._optional, M.nothing() as M.Nothing)
  }
}
...
export type AnyPropertyWithInstance<F extends URIS> = PropertyWithInstance<
  ...
  AnyS,
  Optional | Required,
  M.Maybe<[DefaultConstraint, () => any]>
>

export function prop<S extends AnyS>(_: S): Property<S, Required, M.Nothing> {
  return new Property(_, 'required', M.nothing() as M.Nothing)
}

export type AnyP = Property<AnyS, Required | Optional, M.Maybe<[DefaultConstraint, () => any]>>
export type AnyUP = Property<AnyUS, Required | Optional, M.Maybe<[DefaultConstraint, () => any]>>

export type AnyPOf<U extends URIS> = Property<AnySOf<U>, Required | Optional, M.Maybe<[DefaultConstraint, () => any]>>
export type AnyUPOf<U extends URIS> = Property<AnyUSOf<U>, Required | Optional, M.Maybe<[DefaultConstraint, () => any]>>
  ...
  UnionToIntersection<
    {
      [K in keyof P]: P[K] extends Property<infer S, infer O, infer D>
        ? O extends Optional
          ...
          : D extends M.Just<['decoder' | 'both', any]>
          ? {
              [H in K]?: InputOf<S>
            }
          : {
  ...
  UnionToIntersection<
    {
      [K in keyof P]: K extends TagsFromProps<P>
        ...
        ? O extends Optional
          ...
          : D extends M.Just<['constructor' | 'both', any]>
          ? {
              [H in K]?: CInputOf<S>
            }
          : {
...
export function tagsFromProps(props: PropertyRecord): Record<string, string> {
  ...
  for (const key in props) {
    ...
    if (M.isNothing(props[key]._def) && props[key]._optional === 'required' && isTagS(s)) {
      tags[key] = s.api.tag
    }
  }
  return tags
